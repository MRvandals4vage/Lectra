...
export default function MeetingRoom({ roomId, onLeave }: { roomId: string, onLeave: () => void }) {
  ...
  if (!token) return <div className="h-screen flex flex-col items-center justify-center bg-lectra-background font-outfit text-lectra-text gap-4">
    ...
    <div className="text-center">
      <h2 className="text-2xl font-black uppercase tracking-tighter">Joining Classroom</h2>
      <p className="text-lectra-muted text-[10px] font-black uppercase tracking-widest mt-1">Connecting to the live session...</p>
    </div>
  </div>;
    ...
    >
      ...
      <header className="flex items-center justify-between px-8 py-5 border-b border-lectra-border bg-lectra-sidebar/40 backdrop-blur-3xl sticky top-0 z-50">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-lg font-black tracking-tighter uppercase font-outfit leading-none">Lectra Classroom</h1>
          ...
          <div className="flex-1 overflow-y-auto space-y-8 custom-scrollbar pr-2">
            <div className="bg-lectra-card/50 rounded-3xl p-6 border border-lectra-border transition-all hover:bg-lectra-card group relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-[0.03] group-hover:opacity-5 transition-opacity">
               ...
               <p className="text-[10px] font-black text-lectra-primary uppercase tracking-[0.2em] mb-4">Live Summary</p>
               <p className="text-sm text-lectra-muted leading-relaxed font-medium">
                 {isAnalyzing ? "Analyzing class activity..." : analysis?.summary || "AI insights will appear here as the class continues."}
               </p>
            </div>

            {analysis?.concepts && (
              <div className="space-y-4">
                <p className="text-[10px] font-black text-lectra-muted uppercase tracking-[0.2em]">Key Concepts</p>
            ...
            {activeTab === 'chat' && (
              <div className="space-y-6 h-full flex flex-col">
                <div className="flex-1 space-y-6">
                  ...
                  {messages.length === 0 && (
                    <div className="text-center py-20 opacity-20 italic text-xs font-bold uppercase tracking-widest">No messages yet</div>
                  )}
                </div>
            ...
            {activeTab === 'polls' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center px-1">
                     <span className="text-[10px] font-black text-lectra-muted uppercase tracking-[0.25em]">Class Polls</span>
                  {user?.role === 'teacher' && (
                    <button
                      onClick={() => {
                ...
                <div className="space-y-4">
                  ...
                  {polls.length === 0 && (
                     <div className="text-center py-20 opacity-20 italic text-xs font-black uppercase tracking-widest">No active polls</div>
                  )}
                </div>
              </div>
            )}
  ...
  return (
    <div className="space-y-6">
       <div className="flex items-center justify-between mb-4 px-1">
        <span className="text-[10px] font-black uppercase tracking-[0.25em] text-lectra-muted">Students Online</span>
          ...
          <div key={p.sid} className="flex items-center gap-4 p-4 rounded-[1.5rem] bg-lectra-card/30 border border-lectra-border transition-all hover:bg-lectra-card group">
            ...
            <div className="flex-1 min-w-0">
              ...
              {p.isSpeaking ? (
                ...
                <p className="text-[8px] text-lectra-muted font-bold uppercase tracking-widest mt-2">Silent</p>
              )}
            </div>
            <div className="flex items-center gap-3">
